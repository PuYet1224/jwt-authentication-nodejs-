...
function App() {
  ...
  const handleLogout = async () => {
    if (userData && userData.data && userData.data.shop && userData.data.shop.id) {
      try {
        const response = await fetch('http://localhost:3056/api/shop/logout', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ userId: userData.data.shop.id }),
        });
  ...
  return (
    <div style={{ margin: '20px' }}>
      {userData ? (
        <div>
          <h2>Chào, {userData.data.shop.name}!</h2>
          <p>Email: {userData.data.shop.email}</p>
